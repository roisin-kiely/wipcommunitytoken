...
const Wallet = function ({ children }) {
  ...
  const [loading, setLoading] = useState(false); // To manage loading state for wallet connection
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && !web3) {
      ...
      // Set Web3Modal once on component mount
    }
  }, [web3]);
  ...
  const connect = async function () {
    try {
      setLoading(true); // Start loading when connecting wallet
      ...
      setLoading(false); // Stop loading after the connection attempt
    }
  };
  ...
  const exp = {
    ...
    handleAddComment,
    applyForWip,
    loading, // Include loading state for the UI to display connecting status
  };
